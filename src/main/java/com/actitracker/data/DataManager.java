...
public class DataManager {
  ...
  public static JavaRDD<long[]> withTimestamp(JavaRDD<CassandraRow> data) {
    ...
    public static JavaRDD<double[]> toDoubleLive(JavaRDD<CassandraRow> data) {
    ...
    public static JavaRDD<long[]> withTimestampLive(JavaRDD<CassandraRow> data) {

        // first transform CassandraRDD into a RDD<Map>
        return data.map(CassandraRow::toMap)
                // then build  a double array from the RDD<Map>
                .map(entry -> new long[]{(long) entry.get("timestamp_long"), ((Double) entry.get("y")).longValue()});
    }
